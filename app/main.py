...
from .topics import ReviewDoc, extract_topics
...
async def get_overview(
    ...
    # Fetch reviews with text for topic analysis (shared by Claude and TF-IDF fallback).
    reviews_dicts = await _fetch_reviews_as_dicts(task_id, session)

    # Try Claude first; fall back to local TF-IDF when API key is not configured.
    top_problems, top_praise = await claude_service.generate_top_mentions(reviews_dicts)

    if not top_problems and not top_praise:
        topic_docs = [
            ReviewDoc(id=str(i), text=r["text"], rating=r["rating"])
            for i, r in enumerate(reviews_dicts)
        ]
        loop = asyncio.get_running_loop()
        raw_problems, raw_praise = await loop.run_in_executor(
            None, _run_topic_extraction, topic_docs
        )
        top_problems = [
            TopMention(label=t.label, mentions=t.mentions, examples=t.examples)
            for t in raw_problems
        ]
        top_praise = [
            TopMention(label=t.label, mentions=t.mentions, examples=t.examples)
            for t in raw_praise
...
async def _fetch_reviews_as_dicts(task_id: UUID, session: AsyncSession) -> list[dict]:
...
async def get_task_problems(task_id: UUID, session: AsyncSession = Depends(get_session)):
    task = await _require_task(task_id, session)
    reviews = await _fetch_reviews_as_dicts(task_id, session)
